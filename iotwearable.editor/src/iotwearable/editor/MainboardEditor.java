...
public class MainboardEditor extends GraphicalEditorWithPalette{
	...
	protected void configureGraphicalViewer() {
		...
		getGraphicalViewer().setContextMenu(new MainboardContextMenu(getGraphicalViewer(), getActionRegistry()));
		// D&D
	...
	public class UnwrappingPropertySource implements IPropertySource {
		...
		public void setPropertyValue(Object id, Object value) {
			if(id.equals("id")){
				IDValidator validator = new IDValidator(value.toString(), null);
				if(!validator.isValid()){
					MessageWindow.show("Changing ID", validator.getInvalidMessage());
					return;
				}
				if(!enableChangeIDControl(value)){
					MessageWindow.show("Changing ID", "ID: "+ value + " existed.");
					return;
				}
			}
	...
	protected void createActions() {
		super.createActions();
		IAction action = new CreateMainboardContextMenuAction(this,CreateMainboardContextMenuAction.ID_AddLED, mainboard);
		getActionRegistry().registerAction(action);
		action = new CreateMainboardContextMenuAction(this,CreateMainboardContextMenuAction.ID_AddKeypad4x4, mainboard);
		getActionRegistry().registerAction(action);
		action = new CreateMainboardContextMenuAction(this, CreateMainboardContextMenuAction.ID_AddI2CLCD2004, mainboard);
		getActionRegistry().registerAction(action);
		action = new CreateMainboardContextMenuAction(this,CreateMainboardContextMenuAction.ID_AddBuzzer, mainboard);
		getActionRegistry().registerAction(action);
		action = new CreateMainboardContextMenuAction(this,CreateMainboardContextMenuAction.ID_AddBluetoothHC06, mainboard);
		getActionRegistry().registerAction(action);
		action = new CreateMainboardContextMenuAction(this,CreateMainboardContextMenuAction.ID_AddWifiESP8266, mainboard);
		getActionRegistry().registerAction(action);
	}
	@Override
	...
	protected boolean enableChangeIDControl(Object newid){
		try {
			for(IOControl control : ((Mainboard)getGraphicalViewer().getContents().getModel()).getControls()){
				if(control.getId().equals(newid))
					return false;
			}
			for(ConnectivityControl control : ((Mainboard)getGraphicalViewer().getContents().getModel()).getConnectivities()){
				if(control.getId().equals(newid))
					return false;
			}
		} catch (Exception e) {

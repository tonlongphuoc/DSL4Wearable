...
public class MainboardPalette extends PaletteRoot {
	...
	public MainboardPalette() {
		...
		add(pdInput);
		addEntryInputControl();

		pdOutput = new PaletteDrawer("Output Devices");
		add(pdOutput);
		addEntryOutputControl();
		
		pdConnecttivity = new PaletteDrawer("Connectivity Devices");
		add(pdConnecttivity);
		addEntryConnectivityControl();
		
	}

	// area input control
	private void addEntryInputControl() {
	...
	// area output control
	private void addEntryOutputControl() {
		...
		CreationToolEntry _i2cLCD2004 = new CombinedTemplateCreationEntry("I2C LCD 2004", "I2C LCD 2004",
				new I2CLCD2004Factory(),
				ImageDescriptor.createFromFile(this.getClass(),"icons/i2cLCD1_ESP_LAB.png"),
				ImageDescriptor.createFromFile(this.getClass(),"icons/i2cLCD1_ESP_LAB.png"));
		
		pdOutput.add(_led);
		pdOutput.add(_buzzer);
		pdOutput.add(_i2cLCD2004);
	}
	// area Connectivity control
	
	private void addEntryConnectivityControl(){
